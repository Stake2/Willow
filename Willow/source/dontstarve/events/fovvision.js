...
module.exports = (body, res) => {
    ...
    getClientChannel(channel).then((channel) => {
        if (!channel) return;
        channel.send({ content: `-# O usuário está usando \`${dictionary[body.value.prefab] || body.value.prefab}\` \`${body.value.quantity}\``, embeds: [buildEmbed({ color: 0xCDC4D6, description: `\`🐰\` **· ${body.value.name} \`${body.value.userid}\` tem visão ampla.**` })] }).catch((error) => { print(`[Error] Application error: ${error}`); });
        addCheater(body.value.userid, serverId);
        if (body.value.quantity > 70) {
