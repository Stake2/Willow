...
module.exports = (body, res) => {
    ...
    getClientChannel(channel).then(async (channel) => {
        ...
        channel.send({ embeds: [{ color: 0xC84937, title: `\`🏟️\` · ${boss.name} caiu!`, description: `- Que perigo! Isso foi **intenso**!\n- Foi **${body.value.doer || "Charlie"} quem o derrubou**.\n- Outro(s) **0${players.length - 1}** estão presentes.`, thumbnail: { url: "https://media.discordapp.net/attachments/1270552171041263658/1343495140702027776/image.png?ex=67bd7ada&is=67bc295a&hm=5ce03cab9ab65c4b76ebf9126377720f73b5dfebdbd1a5da5f42ea9bce05a456&=&format=webp&quality=lossless" } }] }).catch((error) => { print(`[Error] Application error: ${error}`); });
