...
async function loginDiscordClient(
  ...
  try {
    ...
    if (retries > 0) {
      ...
      setTimeout(
        () => loginDiscordClient(clientData, retries - 1, delay * 2),
        delay
      );
    } else
      printMajorError(
        `[Error] Failed to connect to Discord client: ${error.message}`
      );
  }
}
...
async function setupDiscord() {
  ...
  const commandKit = new CommandKit({
    ...
    commandsPath: path.join(__dirname, "./discord/commands"),
    eventsPath: path.join(__dirname, "./discord/events"),
  });
  await loginDiscordClient(clientData, commandKit, -1, 5000);
}

async function setClientGuild() {
  clientData.guild = await clientData.client.guilds
...
async function getClientMember(id) {
  ...
  member = await clientData.guild.members.fetch(id).catch((error) => {
    print(`[Error] Application error: ${error}`);
  });
...
async function getClientRole(id) {
  ...
  if (role) return role;
  role = await clientData.guild.roles.fetch(id).catch((error) => {
    print(`[Error] Application error: ${error}`);
  });
...
async function getClientChannel(id) {
  ...
  channel = await clientData.guild.channels.fetch(id).catch((error) => {
    print(`[Error] Application error: ${error}`);
  });
...
async function fetchAllMembers() {
  try {
    print(`[Log] Fetching data: Discord members`);
    await clientData.guild.members.fetch();
  } catch (error) {
    print(`[Error] Application error: ${error}`);
  }
}
