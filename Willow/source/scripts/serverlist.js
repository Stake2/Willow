...
function updateServerDay(serverId, day, season, rift) {
  if (!servers[serverId]) return null;
  servers[serverId].day = day;
  databaseServersUpdateServer(serverId, season, rift);
}
...
function deleteServerOldMessages(server) {
  servers[server].messages = servers[server].messages.filter(message => !message.sent);
}

async function setupServers() {
  const rows = await databaseServersGetServers();
  for (const row of rows) {
    let server = { id: row.id, identity: row.identity, channel: row.discord_channel, messages: [], cooldown: 0, day: 1, messageCount: 0 };
